...
float *my_parent(int n_operations, int NPROC, struct operation *operations, int queues[]){
    ...
    //###################################################
                                                      //#
    for (i = 0; i < NPROC; i++) {
        msgrcv(queues[i], &cmb1, sizeof(struct custom_msgbuf1) - sizeof(long), 1, 0);
    }
                                                      //#
    //###################################################
    ...
    for(op_id = 0; op_id < n_operations; op_id++)
    {
        ...
        if(!child_isFree[i]) {
            ...
            msgrcv(queues[i], &cmb3, sizeof(struct custom_msgbuf3)-sizeof(long), 3, 0);
            res = cmb3.res;
            res_id = cmb3.id;
        ...
        cmb2.op_id = op_id;
        msgsnd(queues[i], &cmb2, sizeof(struct custom_msgbuf2)-sizeof(long), 0);
