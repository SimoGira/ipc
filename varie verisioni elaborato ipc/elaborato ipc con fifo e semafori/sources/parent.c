...
float *my_parent(int n_operations, int NPROC, struct operation *operations, int my_semaphores[], int fd1, int fd2){
    ...
    while (childs_ready < NPROC) {
        ...
        childs_ready++;
        write(fd1, &childs_ready,sizeof(int));
    }
                                                      //#
    //###################################################
    ...
    for(op_id = 0; op_id < n_operations; op_id++)
    {
        ...
        if(!child_isFree[i]) {
            ...
            read(fd2, &current_result, sizeof(struct result));
            res = current_result.val;
            res_id = current_result.id;
        ...
        write(fd1, &current_operation,sizeof(struct operation));
    ...
    for(i = 0; i < NPROC; i++)
    {
        ...
        current_operation.id = i;
        current_operation.val1 = operations[i].val1;
        current_operation.val2 = operations[i].val2;
        current_operation.operator = operations[i].operator;
