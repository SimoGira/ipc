    ...
    *(sem_init + 2) = 0;
    ...
    for (i = 0; i < NPROC; i++)
    {
        ...
        if (pid < 0) {
            ...
            if ((fifo_fd1 = open("fifo1", O_RDONLY))==-1){
                syserr(argv[0], "open failure");
            }
            if ((fifo_fd2 = open("fifo2", O_WRONLY))==-1){
                syserr(argv[0], "open failure");
            }
            child(i, NPROC, my_semaphores, fifo_fd1, fifo_fd2);
            free(operations);
    ...
    if(pid != 0)
    {
        if ((fifo_fd1 = open("fifo1", O_WRONLY))==-1){
            syserr(argv[0], "open failure");
        }
        
        if ((fifo_fd2 = open("fifo2", O_RDONLY))==-1){
            syserr(argv[0], "open failure");
        }
        
        float *results = my_parent(n_operations, NPROC, operations, my_semaphores, fifo_fd1, fifo_fd2);
