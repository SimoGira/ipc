...
float *my_parent(int n_operations, int NPROC, struct operation *operations, int my_semaphores[], int msqid){
    ...
    while (childs_ready < NPROC) {
        msgrcv(msqid, &cmb1, sizeof(struct custom_msgbuf1) - sizeof(long), 1, 0);
        childs_ready++;
    ...
    for(op_id = 0; op_id < n_operations; op_id++)
    {
        ...
        if(!child_isFree[i]) {
            ...
            msgrcv(msqid, &cmb3, sizeof(struct custom_msgbuf3)-sizeof(long), 3, 0);
            res = cmb3.res;
            res_id = cmb3.id;
        ...
        cmb2.mtype = 2;
        cmb2.val1 = operations[op_id].val1;
        cmb2.val2 = operations[op_id].val2;
        cmb2.op = operations[op_id].operator;
        cmb2.op_id = op_id;
        msgsnd(msqid, &cmb2, sizeof(struct custom_msgbuf2)-sizeof(long), 0);
    ...
    for(i = 0; i < NPROC; i++)
    {
        ...
        cmb2.mtype = 2;
        cmb2.val1 = operations[i].val1;
        cmb2.val2 = operations[i].val2;
        cmb2.op = operations[i].operator;
        cmb2.op_id = i;
        msgsnd(msqid, &cmb2, sizeof(struct custom_msgbuf2)-sizeof(long), 0);
