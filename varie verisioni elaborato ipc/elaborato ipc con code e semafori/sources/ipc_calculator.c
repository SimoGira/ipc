...
int main(int argc, char *argv[]){
    ...
    *(sem_init + 2) = 0;
    ...
    if((msqid = msgget(ftok(argv[0], 'a'), 0666 | IPC_CREAT)) < 0){
        syserr(argv[0], "cannot create msgqid");
    }
    ...
    for (i = 0; i < NPROC; i++)
    {
        ...
        if (pid < 0) {
            ...
            child(i, NPROC, my_semaphores, msqid);
            free(operations);
            exit(0);
        } else {
    ...
    if(pid != 0)
    {
        ...
        float *results = my_parent(n_operations, NPROC, operations, my_semaphores, msqid);
        
        /* delete queues */
        if(msgctl(msqid, IPC_RMID, NULL) == -1)
            syserr(argv[0], "msgctl failed");
