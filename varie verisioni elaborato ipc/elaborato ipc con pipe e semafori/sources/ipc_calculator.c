...
int main(int argc, char *argv[]){
    ...
    *(sem_init + 2) = 0;
    ...
    /******************* PIPE ********************/
    int p1[2]; /* from parent to childs */
    int p2[2]; /* from childs to parent */
    ...
    for (i = 0; i < NPROC; i++)
    {
        ...
        if (pid < 0) {
            ...
            close(p2[0]);
            close(p1[1]);
            child(i, NPROC, my_semaphores, p1, p2);
            free(operations);
            exit(0);
        } else {
    ...
    if(pid != 0)
    {
        ...
        float *results = my_parent(my_semaphores, n_operations, NPROC, operations, p1, p2);
