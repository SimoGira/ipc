...
float *my_parent(int my_semaphores[], int n_operations, int NPROC, struct operation *operations, int p1[], int p2[]){
    ...
    while (childs_ready < NPROC) {
        
        write(p1[1], &childs_ready, sizeof(childs_ready));
        
        read(p2[0], &childs_ready, sizeof(childs_ready));
    }
    ...
    for(op_id = 0; op_id < n_operations; op_id++)
    {
        ...
        if(!child_isFree[i]) {
            ...
            read(p2[0], &res_id, sizeof(res_id));
            read(p2[0], &res, sizeof(res));
        ...
        write(p1[1], &op_id, sizeof(op_id));
        write(p1[1], &operations[op_id].val1, sizeof(operations[op_id].val1));
        write(p1[1], &operations[op_id].val2, sizeof(operations[op_id].val2));
        write(p1[1], &operations[op_id].operator, sizeof(operations[op_id].operator));
    ...
    for(i = 0; i < NPROC; i++)
    {
        ...
        write(p1[1], &operations[i].val1, sizeof(operations[i].val1));
        write(p1[1], &operations[i].val2, sizeof(operations[i].val2));
        write(p1[1], &operations[i].operator, sizeof(operations[i].operator));
