...
*/
    ...
    /******************* PIPE ********************/
    int write_pipes[NPROC];
    int read_pipes[NPROC];
    ...
    for (i = 0; i < NPROC; i++)
    {
        ...
        if (pid < 0) {
            ...
            close(new_pipe1[1]);
            close(new_pipe2[0]);
            child(i, NPROC, new_pipe1, new_pipe2);
            free(operations);
            exit(0);
        } else {
            ...
            write_pipes[i] = new_pipe1[1];
            read_pipes[i] = new_pipe2[0];
                                                   //#
            //########################################
        }
    }
    ...
    if(pid != 0)
    {
        ...
        float *results = my_parent(n_operations, NPROC, operations, read_pipes, write_pipes);
